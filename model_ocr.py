...
def my_data_gen(x,rotation_range=0.,
                        width_shift_range=0.,
                        ...
                        vertical_flip=False,
                        seed=None):
    x =  random_transform(x, rotation_range,
                            width_shift_range,
                            height_shift_range,
                            shear_range,
                            zoom_range,
...
def random_transform(x, rotation_range=0.,
        ...
        if rotation_range:
            theta = cp.deg2rad(cp.random.uniform(-rotation_range, rotation_range))
        else:
            theta = 0

        if height_shift_range:
            tx = cp.random.uniform(-height_shift_range, height_shift_range)
        ...
        if width_shift_range:
            ty = cp.random.uniform(-width_shift_range, width_shift_range)
        ...
        if shear_range:
            shear = cp.deg2rad(cp.random.uniform(-shear_range, shear_range))
        else:
            shear = 0
        ...
        if zoom_range[0] == 1 and zoom_range[1] == 1:
            ...
            zx, zy = cp.random.uniform(zoom_range[0], zoom_range[1], 2)

        transform_matrix = None
        if theta != 0:
            rotation_matrix = cp.array([[float(cp.cos(theta)), -float(cp.sin(theta)), 0],
                                        [float(cp.sin(theta)), float(cp.cos(theta)), 0],
                                        [0, 0, 1]])
            transform_matrix = rotation_matrix

        if tx != 0 or ty != 0:
            shift_matrix = cp.array([[1, 0, float(tx)],
                                     [0, 1, float(ty)],
                                     [0, 0, 1]])
            transform_matrix = shift_matrix if transform_matrix is None else cp.dot(transform_matrix, shift_matrix)

        if shear != 0:
            shear_matrix = cp.array([[1, -float(cp.sin(shear)), 0],
                                    [0, float(cp.cos(shear)), 0],
                                    [0, 0, 1]])
            transform_matrix = shear_matrix if transform_matrix is None else cp.dot(transform_matrix, shear_matrix)

        if zx != 1 or zy != 1:
            zoom_matrix = cp.asarray(np.array([[float(zx), 0, 0],
                                    [0, float(zy), 0],
                                    [0, 0, 1]]))
            transform_matrix = zoom_matrix if transform_matrix is None else cp.dot(transform_matrix, zoom_matrix)
...
def apply_transform(x,
    ...
    """
    x = cp.rollaxis(x, channel_axis, 0)
    final_affine_matrix = transform_matrix[:2, :2].get()
    final_offset = transform_matrix[:2, 2].get()
    # print(x.__class__)
    x_ = x.get()
    # print(x_.shape)
    # import time
    # start = time.time()
    channel_images = cp.array([ndi.interpolation.affine_transform(
        x_channel,
        final_affine_matrix,
        final_offset,
        order=1,
        mode=fill_mode,
        cval=cval) for x_channel in x_])
    # print("affine_transform", time.time()-start)
    x = cp.stack(channel_images, axis=0)
    x = cp.rollaxis(x, 0, channel_axis + 1)
    return x

def transform_matrix_offset_center(matrix, x, y):
    o_x = float(x) / 2 + 0.5
    o_y = float(y) / 2 + 0.5
    offset_matrix = cp.array([[1, 0, o_x], [0, 1, o_y], [0, 0, 1]])
    reset_matrix = cp.array([[1, 0, -o_x], [0, 1, -o_y], [0, 0, 1]])
    transform_matrix = cp.dot(cp.dot(offset_matrix, matrix), reset_matrix)
    return transform_matrix

def flip_axis(x, axis):
    x = cp.asarray(x).swapaxes(axis, 0)
    x = x[::-1, ...]
    x = x.swapaxes(0, axis)
    return x
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    # print('shape image_fake_temple')
    # print(image_fake_temple.shape)
    ...
    if augment:
        if random.randint(0, 1):
            ...
            image_fake_temple = np.concatenate((image,mask), axis=2)
            # print("concatenate", time.time()-start)
    #         if random.randint(0, 1):
    #             image = np.fliplr(image)
    #             mask = np.fliplr(mask)
            # start = time.time()
            image_fake_temple_gpu = cp.asarray(image_fake_temple)
            result_transform_gpu = my_data_gen(image_fake_temple_gpu,
                        rotation_range=10.,
                        ...
                        horizontal_flip=False,
                        vertical_flip=False)
            # print("my_data_gen", time.time()-start)
    #         print('JOJO shape result_transform')
    #         print(result_transform.shape)

            # start = time.time()
            result_transform = cp.asnumpy(result_transform_gpu)
            image = result_transform[:,:,0:3]

    #         cv2.imwrite("JOJO_IMAGE.jpg",image)
    #         cv2.imwrite("ORIGINAL_JOJO_{}.jpg".format(image_id), image)
    #         print(image.shape)

            mask = result_transform[:,:,3:image_fake_temple.shape[2]]
        # print("split", time.time()-start)

# #         Draw mask into original image --- validate transfomation
# #         Generate random colors
        
#         tmp_image_mask = image.astype(np.uint32).copy()
#         N = mask.shape[2]
...
#             color = colors[i]
#             mask_test = mask[:, :, i]
#             tmp_image_mask = apply_mask(tmp_image_mask, mask_test, color)
#         print(tmp_image_mask.shape)
#         print("JOJO_MASK")

#         print(type(tmp_image_mask))
        
#         print (image_id)
#         plt.figure(figsize=(18, 18))
#         plt.imshow(tmp_image_mask)
#         plt.show()
...
def data_generator(dataset, config, shuffle=True, augment=True, random_rois=0,
    ...
    bounding box deltas, and maskszz.
